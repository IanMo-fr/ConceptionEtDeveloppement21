...
public class Diversité {
    ...
    /**
     * Constructeur par défaut de Diversité
     */
    public Diversité() {
    ...
    public void Verification(List<List<String>> ListeDS, int k, int l) {
        ...
        List<String> Liste_données=new ArrayList<>();  //Liste dans laquelle on va ajouter les données de chaque groupe, qu'on effacera pour y mettre celles du prochain groupe
        ...
        for (int a = 1; a <= taille_listeDS-k-1; a += k) {  //On parcourt de groupe en groupe, sans oublier que le premier élément est un label de catégorie
            ...
            Liste_données = new ArrayList<>();                      //On vide la liste
            ...
            for (int b = 0; b < k; b++) {
                ...
                if (Liste_données.contains(val) == false) {         //On vérifie si l'élément du groupe était déjà présent ou non dans les éléments du groupe parcouru
                    compteur_differences +=1;                       //Si présence d'un nouvel élément -> on incrémente le compteur
                }
                Liste_données.add(ListeDS.get(0).get(a + b));       //On ajoute l'élément testé à la liste contenant les éléments déjà testés d'un groupe
        ...
        for (int c=taille_listeDS-((taille_listeDS-1)%k);c<taille_listeDS;c++) {  //On va chercher l'indice prochain élément à traiter, s'il reste des éléments et on effectue cette boucle pour chacun des éléments du dernier groupe encore à tester
           ...
           if (Liste_données.contains(val) == false) {  //On regarde si c'est un nouvel élément
               compteur_differences +=1;                //On incrémente le compteur en fonction
           }
           Liste_données.add(ListeDS.get(0).get(c));   //On l'ajoute pour continuer les tests
        }
